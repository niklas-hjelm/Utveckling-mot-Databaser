...
SET
    UserName = LOWER(
        SUBSTRING(FirstName, 0, 3) + SUBSTRING(LastName, 0, 3)
    )
select
    SUBSTRING('Fredrik', 1, 2)
UPDATE
    Users
SET
    UserName = LOWER(SUBSTRING(FirstName, 1, 2)) + LOWER(SUBSTRING(LastName, 1, 2));
...
select
    left('Fredrik', 3)
select
    right('Fredrik', 3) -- Uppdatera (kopia p�) tabellen airports s� att alla null-v�rden i kolumnerna
...
SELECT
    ...
    CASE
        ...
        ELSE 'No'
    END AS 'ContainsSymbol' INTO Elements3
FROM
    Elements;
...
select
    ...
    end as "Yes/No" --     into FinishedElement2
from
    Elements;
...
select
    *
from
    FinishedElement2 -- Kopiera tabellen Colors till Colors2, men skippa kolumnen �Code�. G�r
...
ADD
    Code VARBINARY(8)
ALTER TABLE
    Colors2 DROP COLUMN Code;

UPDATE
    Colors2
SET
    Code = CONVERT(VARBINARY(1), Red) + CONVERT(VARBINARY(1), Green) + CONVERT(VARBINARY(1), Blue)
SELECT
    Name,
    Red,
    Green,
    Blue,
    + '#' + CONVERT(VARCHAR(1000), Code, 2) AS Code
from
    Colors2;
...
select
    *
from
    types2 declare @Date DateTime2 = '2018-12-31 09:00:00'
select
    Integer,
    String,
